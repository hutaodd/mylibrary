...
var UpdateTimeOnEditSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  addEnableNumberProperties() {
    new import_obsidian5.Setting(this.containerEl).setName("启用数字属性类型").setDesc(
      "当使用数字格式（如 Unix 时间戳）时，将数字分配给日期属性（而不是字符串）。"
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.enableNumberProperties).onChange(async (newValue) => {
