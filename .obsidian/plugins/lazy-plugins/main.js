...
var require_manifest = __commonJS({
  "manifest.json"(exports, module2) {
    module2.exports = {
      ...
      version: "1.0.18",
      minAppVersion: "1.6.0",
      description: "在启动时延迟加载插件，以便将应用程序的启动时间缩短到亚秒级。",
...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  buildDom() {
    ...
    new import_obsidian.Setting(this.containerEl).setName("单独的桌面/移动设备配置").setDesc(`如果您希望根据使用的是台式机还是移动设备来设置不同的配置，请启用此选项。以下所有设置都可以在台式机和移动设备上进行不同的配置。您当前使用的是 ${this.lazyPlugin.device} 设置。`).addToggle((toggle) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("延迟加载器设置").setHeading();
    Object.entries({
      shortDelaySeconds: "短延迟 (单位秒)",
      longDelaySeconds: "长延迟 (单位秒)"
    }).forEach(([key, name]) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("新插件的默认启动类型").addDropdown((dropdown) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("显示插件描述").addToggle((toggle) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("一次性设置所有插件的延迟").addDropdown((dropdown) => {
    ...
    new import_obsidian.Setting(this.containerEl).setName("插件").setHeading().setDesc("过滤依据：").then((setting) => {
          ...
          const timeout = setTimeout(async () => {
            ...
            if (!((_b2 = (_a2 = obsidian.plugins) == null ? void 0 : _a2[pluginId]) == null ? void 0 : _b2._loaded)) {
              if (this.data.showConsoleLog) {
                console.log(`在 ${startupType} 延迟后启动 ${pluginId}`);
              }
              await obsidian.enablePlugin(pluginId);
            }
          }, seconds * 1e3 + delay);
  ...
  async onunload () {
    ...
    for (const plugin of this.manifests) {
      ...
      if (startupType !== LoadingMethod.disabled) {
        ...
        console.log(`将 ${plugin.id} 设置回即时启动`)
      }
    }
  } */
};
